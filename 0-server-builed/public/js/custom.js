...
function toggleFavoriteItem(id) {
    ...
    if (exist)
        currentFavorites = currentFavorites.filter((fID) => fID != id);
    else
...
function updateFavorites(ids = []) {
    ...
    const all_totalFavoriteItems = document.querySelectorAll(".totalFavoriteItems");
...
const checkCartExist = (id) => getCurrentCart()?.some((d) => d?.product?.id == id);
...
function toggleCartItem(stringifyProduct, action = "") {
    ...
    if (exist) {
        if (action !== "buynow") {
            currentCart = currentCart.filter((cartItem) => cartItem?.product?.id != id);
        }
    }
    else {
        currentCart.push({
...
function handleQuantity(input, index) {
    ...
    const in_stock = getCurrentCart()?.find((_, i) => i === index)?.product
        ?.in_stock;
    if (quantity > in_stock || quantity < 1)
        return;
    const cart = getCurrentCart()?.map((item, i) => i === index ? { ...item, quantity } : item);
    updateCart(cart);
}
function updateCart(cart) {
    ...
    const quantitiesByProductInput = document.getElementById("quantitiesByProduct");
    ...
    const all_cartPurchaseSubTotalInput = document.querySelectorAll(".cartPurchaseSubTotalInput");
    ...
    all_cartPurchaseSubTotalInput?.forEach((ele) => (ele.value = purchaseSubTotal));
    ...
    quantitiesByProductInput &&
        quantitiesByProductInput?.setAttribute("value", JSON.stringify(quantitiesByProduct));
    syncCartButtons();
...
function displayCartProducts(latestCart = []) {
    ...
    if (latestCart && cartProducts) {
        ...
        <div class="flex flex-wrap items-center -mx-4">
          <div class="w-full px-4 md:w-1/3">
            <div class="w-full md:w-24 flex items-center">
              <img src="${item?.product?.images[0]}" alt="" class="w-20 rounded">
            </div>
          </div>
          <div class="w-2/3 px-4">
            <a href="/item/${item?.product?.slug}" class="mb-2 text-lg font-bold block text-blue-600 hover:underline"> ${item?.product?.name} </a> 
      ...
      <div class="hidden px-4 lg:block lg:w-2/12">
        <p class="text-lg font-bold text-orange-600 dark:text-gray-400"> ৳ ${item?.product?.sale_price} </p>
        <span class="text-xs text-gray-500 line-through dark:text-gray-400"> ৳${(item?.product?.sale_price / 100) * 10 + item?.product?.sale_price} </span>
      </div>

      <div class="w-3/12">
      <div class="pr-10">
        <div class="rounded">
          <input onkeyup="handleQuantity(this, ${index})" onchange="handleQuantity(this, ${index})" value="${item?.quantity}" type="number" class="p-1 w-14 text-center rounded focus:outline-none focus:ring border" placeholder="quantity" />
        </div>

        <div>
        <p class="text-lg font-bold text-orange-600"> ৳${item?.product?.sale_price * item?.quantity} </p>
        </div>
        </div>
      </div>
     
      <div class="w-1/12 text-center">
        <button data-id="${item?.product?.id}" title="Remove from cart" class="removeCartBtn text-red-500 text-xl transform transition-all hover:scale-105 focus:ring bg-gray-300 px-3 py-2 rounded shadow"> 
