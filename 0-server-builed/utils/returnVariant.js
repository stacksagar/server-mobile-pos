...
function returnVariant(prev_variants, properties) {
    if (!properties?.imei || prev_variants?.length < 1)
        return prev_variants;
    const new_variants = prev_variants.map((variant) => {
        if (variant.processor === properties.processor &&
